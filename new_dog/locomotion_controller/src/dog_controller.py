...
from predictive_support_polygon import *
...
class Dog():
    def __init__(self):
        ...
        self.last_gait = 0
        self.gait_num = 0
        self.Polygon_machine = [Polygon_machine() for i in range(4)]

    def Predictive_Support_Polygon(self):
        # update machine state
        ground_leg=[]
        last_ground_leg=[]
        next_ground_leg=[]
        for k in range(4):
            if self._statemachine._gait.get_schedualgroundLeg()[k] == 1:
                ground_leg.append(k)
            if self._statemachine._gait.get_nextschedualgroundLeg()[k] == 1:
                next_ground_leg.append(k)
            if self._statemachine._gait.get_lastschedualgroundLeg()[k] == 1:
                last_ground_leg.append(k)

        for k in range(4):
            self.Polygon_machine[k].phases = copy.deepcopy(
                self._statemachine._gait.Gait_currentTime / self._statemachine._gait.Gait_time[
                    self._statemachine._gait.Gait_index])

            # phase adjustment
            if k in ground_leg and k in next_ground_leg:
                self.Polygon_machine[k].phases = 0.5
            if k in ground_leg and k in last_ground_leg:
                if self.Polygon_machine[k].phases < 0.5:
                    self.Polygon_machine[k].phases = 0.5

            self.Polygon_machine[k].leg_idx = k
            self.Polygon_machine[k].foot_point = copy.deepcopy(self.footpoint)

            if k in ground_leg:
                self.Polygon_machine[k].touch_ground = True
            else:
                self.Polygon_machine[k].touch_ground = False

            self.Polygon_machine[k].get_Weights_update()

        # get COM position
        self.Polygon_machine[0].get_virtual_point(self.Polygon_machine[2].Weights, self.Polygon_machine[1].Weights)
        self.Polygon_machine[1].get_virtual_point(self.Polygon_machine[0].Weights, self.Polygon_machine[3].Weights)
        self.Polygon_machine[2].get_virtual_point(self.Polygon_machine[3].Weights, self.Polygon_machine[0].Weights)
        self.Polygon_machine[3].get_virtual_point(self.Polygon_machine[1].Weights, self.Polygon_machine[2].Weights)

        COM_x = (self.Polygon_machine[0].virtual_point[0] + self.Polygon_machine[1].virtual_point[0] +
                 self.Polygon_machine[2].virtual_point[0] + self.Polygon_machine[3].virtual_point[0]) / 4
        COM_y = (self.Polygon_machine[0].virtual_point[1] + self.Polygon_machine[1].virtual_point[1] +
                 self.Polygon_machine[2].virtual_point[1] + self.Polygon_machine[3].virtual_point[1]) / 4
    ...
    def getTarget_Force(self):
        ...
        if abs(self.rpy[2][0]) >= np.pi / 2 and abs(target_rpy[2][0]) >= np.pi and self.rpy[2][0] * target_rpy[2][
            0] < 0:
    ...
    def statemachine_update(self):
        ...
        if self._statemachine._gait.Gait_currentTime == 0:
            ...
            self.targetstates = self.getTargetstate(0.01, int(_gait_time / 0.01),
                                                    last_targetstate=self.last_targetstate)
            last_index = self._statemachine._gait.get_lastindex()
            for i in range(4):
                if (self._statemachine._gait.Gait_phase[_gait_index][i] != 1) and (
                        self._statemachine._gait.Gait_phase[last_index][i] == 1):
        ...
        for i in range(4):
            if self.schedualgroundLeg[i] != 1:
                phase_diff = self._statemachine._gait.Gait_phase[next_index][i] - \
                             self._statemachine._gait.Gait_phase[_gait_index][i]
        ...
        if self._statemachine._gait.Gait_currentTime >= _gait_time:
            ...
            self._statemachine.phase = copy.copy(
                self._statemachine._gait.Gait_phase[self._statemachine._gait.Gait_index])
    ...
    def Force_calculation(self):
        ...
        self.force_list, self.forceerror = OSQP_solve(self.footpoint, self._statemachine._gait.Gait_phase[
            self._statemachine._gait.Gait_index], _forceTorque)
        for i in range(4):
            if self._statemachine.phase[i] >= 0.95 and \
                    self._statemachine._gait.Gait_phase[self._statemachine._gait.Gait_index][i] != 1:
        ...
        for i in range(4):
            if self.schedualgroundLeg[i] == 0:
                ...
                final_point += np.array(
                    [[0], [self._statemachine._gait.Gait_pacePropotion * swing_time * _target_vel[1][0]], [0]])
                if USE_RAIBERT_HEURISTIC:
                    final_point += self._statemachine._gait.Gait_pacePropotion * swing_time * (
                            1 - self._statemachine.phase[i]) * np.dot(np.linalg.inv(self.TF_mat),
                                                                      (self.body_vel - self.target_vel))
                _targte_swingpos, _targte_swingvel, _targte_swingacc = gait_swingLeg(self._statemachine.phase[i],
                                                                                     swing_time,
                                                                                     self.init_SWINGfootpoint[i],
                                                                                     final_point)
